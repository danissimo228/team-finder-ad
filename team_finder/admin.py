...
class StatsAdminSite(AdminSite):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "request-stats/",
                self.admin_view(self.request_stats_view),
                name="request_stats",
            ),
        ]
    ...
    def request_stats_view(self, request):
        ...
        top_urls = [
            {"url": url, "count": count} for url, count in url_counter.most_common(10)
        ]
        ...
        errors = [
            entry
            for entry in stats
            if entry.get("error") or entry.get("status_code", 200) >= 400
        ]
        error_rate = (len(errors) / total_requests * 100) if total_requests > 0 else 0

        # Пользователи
        users_counter = Counter(
            entry.get("username", "Аноним") for entry in stats if entry.get("username")
        )
        popular_users = [
            {"username": user, "count": count}
            for user, count in users_counter.most_common(5)
        ]
